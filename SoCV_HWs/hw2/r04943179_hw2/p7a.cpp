...
BddNode
BddMgr::restrict(const BddNode& f, BddNode& c)
{
   assert(c != BddNode::_zero);
   if( c == BddNode::_one ) return f;
   if( f == BddNode::_one || f == BddNode::_zero ) return f;
   
   unsigned thisLevel = c.getLevel();
   BddNode c_left = c.getLeftCofactor(thisLevel);
   BddNode c_right = c.getRightCofactor(thisLevel);
   BddNode f_left = f.getLeftCofactor(thisLevel);
   BddNode f_right = f.getRightCofactor(thisLevel);

   if( c_right == BddNode::_zero ) return BddMgr::restrict(f_left, c_left);
   if( c_left == BddNode::_zero ) return BddMgr::restrict(f_right, c_right);
   if( f_left == f_right ) {
      BddNode c_exist = c_left | c_right;
      return BddMgr::restrict(f, c_exist);
   }
   BddNode a = getSupport(thisLevel);
   return ((~a & BddMgr::restrict(f_right, c_right)) | (a & BddMgr::restrict(f_left, c_left)));
}

   ...
   public:
      ...
      void report(const bool& repTime, const bool& repMem) {
         ...
         if (repMem) {
            ...
            Msg(MSG_IFO) << "Peak memory used   : " << fixed << setprecision(4) << _peakMem << " M Bytes" << endl;
            Msg(MSG_IFO) << "Total memory used  : " << fixed << setprecision(4) << _totalMem << " M Bytes" << endl;
            Msg(MSG_IFO) << "Current memory used: " << fixed << setprecision(4) << _currentMem << " M Bytes" << endl;
         }
      }

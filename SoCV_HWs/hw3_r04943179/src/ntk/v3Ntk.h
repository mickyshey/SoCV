...
class V3Ntk
{
   public : 
      ...
      const bool isBddBuilt() const { return _isBddBuilt; }
      const bool setBddOrder(V3NtkHandler* const, const bool&) const;
      void buildNtkBdd();
      void buildBdd(const V3NetId&);
      void printFaninRec(V3NtkHandler* const, const V3NetId&, const int&, const int&);
      void reportOutputBdd();
	  void reportOutputBdd(const uint32_t& i);
      /* END OF MODIFICATION */
   protected : 
      ...
      bool           _isBddBuilt;   // MODIFICATION FOR SoCV BDD

      // MODIFICATION FOR SoCV BDD
      // put fanins of a net into a vector in topological order
      void dfsOrder(const V3NetId&, V3NetVec&);
};
